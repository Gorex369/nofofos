...
class AttendanceConsideration(db.Model):
    """An auditable attendance-credit request linked to a single student activity."""
    __tablename__ = "attendance_considerations"
    __table_args__ = (db.UniqueConstraint("activity_id", name="uq_activity_attendance_consideration"),)

    id = db.Column(db.Integer, primary_key=True)
    activity_id = db.Column(db.Integer, db.ForeignKey("academic_activities.id"), nullable=False)
    student_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    subject_id = db.Column(db.Integer, db.ForeignKey("subjects.id"), nullable=False)
    requested_attendance_units = db.Column(db.Integer, nullable=False)
    authorized_attendance_units = db.Column(db.Integer, nullable=True)
    status = db.Column(db.String(20), nullable=False, default="Pending")
    request_note = db.Column(db.String(255), nullable=True)
    review_note = db.Column(db.String(1000), nullable=True)
    ...
    activity = db.relationship("AcademicActivity", backref="attendance_consideration")
    student = db.relationship("User", foreign_keys=[student_id], backref="attendance_considerations")
    subject = db.relationship("Subject", backref="attendance_considerations")
