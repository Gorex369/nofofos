...
def test_official_timetable_versioning_matching_and_security():
    ...
    try:
        ...
        with app.test_request_context():
            pass
        with app.app_context():
            timetable = OfficialTimetable.query.filter_by(
                uploaded_by_id=created_user_ids[1], original_filename="official.pdf"
            ).one()
        ...
        with app.app_context():
            second = OfficialTimetable.query.filter_by(
                uploaded_by_id=created_user_ids[1], original_filename="revised.pdf"
            ).one()
        ...
        with app.app_context():
            timetables = OfficialTimetable.query.filter(
                OfficialTimetable.uploaded_by_id.in_(created_user_ids)
            ).all()
