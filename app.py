...
from models import AcademicActivity, AcademicReview, Assessment, Attendance, AttendanceAuthorization, AttendanceConsideration, EvidenceAttachment, FacultyAssignment, OfficialTimetable, Subject, TimetableEntry, User, VerificationAudit
...
def dashboard():
    ...
    overall_attendance = attendance_summary(Attendance.query.filter_by(user_id=user.id).all(), approved_attendance_units(user.id))
    ...
    for subject in subjects:
        ...
        attendance_result = attendance_summary(
            Attendance.query.filter_by(user_id=user.id, subject_id=subject.id).all(),
            approved_attendance_units(user.id, subject.id),
        )
...
def coordinator_activity_detail(activity_id):
    ...
    return render_template("coordinator_activity_detail.html", activity=activity, audits=audits, consideration=AttendanceConsideration.query.filter_by(activity_id=activity.id).first())
...
def faculty_activity_detail(activity_id):
    ...
    audits = VerificationAudit.query.filter_by(target_type="AcademicActivity", target_id=activity.id).order_by(VerificationAudit.created_at.desc()).all()
    return render_template("faculty_activity_detail.html", activity=activity, authorizations=authorizations, audits=audits, consideration=AttendanceConsideration.query.filter_by(activity_id=activity.id).first())
...
def activity_detail(activity_id):
    ...
    return render_template("activity_detail.html", activity=activity, consideration=AttendanceConsideration.query.filter_by(activity_id=activity.id).first())


@app.route("/activities/<int:activity_id>/attendance-consideration", methods=["POST"])
@login_required
def request_attendance_consideration(activity_id):
    activity = AcademicActivity.query.filter_by(id=activity_id, user_id=session["user_id"]).first_or_404()
    consideration = AttendanceConsideration.query.filter_by(activity_id=activity.id).first()
    requested_units = request.form.get("requested_attendance_units", type=int)
    request_note = request.form.get("request_note", "").strip() or None
    if consideration is not None:
        error = "An attendance consideration request already exists for this activity."
    elif activity.subject_id is None:
        error = "Attendance consideration requires an activity linked to one of your subjects."
    elif requested_units is None or not 1 <= requested_units <= 20:
        error = "Request between 1 and 20 attendance units."
    elif len(request_note or "") > 255:
        error = "Supporting details must be 255 characters or fewer."
    else:
        error = None
    upload = request.files.get("proof_file")
    file_info = None
    if error is None and upload and upload.filename:
        file_info, error = validate_evidence_file(upload)
    if error:
        return render_template("activity_detail.html", activity=activity, consideration=consideration, form_error=error), 400

    consideration = AttendanceConsideration(
        activity_id=activity.id, student_id=activity.user_id, subject_id=activity.subject_id,
        requested_attendance_units=requested_units, request_note=request_note, status="Pending",
    )
    storage_path = None
    try:
        ...
        if file_info:
            ...
            db.session.add(EvidenceAttachment(
                activity_id=activity.id, user_id=activity.user_id, original_filename=file_info["original_filename"],
                stored_filename=stored_filename, storage_path=storage_path, mime_type=file_info["mime_type"],
                file_size=file_info["file_size"], description="Supporting proof for attendance consideration" + (f": {request_note}" if request_note else ""), verification_status="Pending",
            ))
        db.session.add(VerificationAudit(actor_user_id=activity.user_id, actor_role="student", target_type="AcademicActivity", target_id=activity.id, action="ATTENDANCE_CONSIDERATION_REQUESTED", note=request_note))
        db.session.commit()
    except Exception:
        db.session.rollback()
        if storage_path and os.path.isfile(storage_path):
            os.remove(storage_path)
        raise
    flash("Attendance consideration submitted for review.", "success")
    return redirect(url_for("activity_detail", activity_id=activity.id))


def attendance_consideration_error(activity, consideration, message, template, **context):
    context.update(activity=activity, consideration=consideration, form_error=message)
    return render_template(template, **context), 400


def decide_attendance_consideration(activity, reviewer, action, template, **context):
    consideration = AttendanceConsideration.query.filter_by(activity_id=activity.id).first_or_404()
    if activity.verification_status != "Verified":
        return attendance_consideration_error(activity, consideration, "The activity must be verified before attendance can be considered.", template, **context)
    if consideration.status != "Pending":
        return attendance_consideration_error(activity, consideration, "This attendance consideration has already been decided.", template, **context)
    note = request.form.get("note", "").strip() or None
    if action == "Authorized":
        units = request.form.get("authorized_attendance_units", type=int)
        if units is None or not 1 <= units <= consideration.requested_attendance_units or len(note or "") > 1000:
            return attendance_consideration_error(activity, consideration, "Grant between 1 and the requested number of units; notes must be 1000 characters or fewer.", template, **context)
        consideration.authorized_attendance_units = units
        audit_action = "ATTENDANCE_CREDIT_GRANTED"
    else:
        if not note or len(note) > 1000:
            return attendance_consideration_error(activity, consideration, "A rejection reason is required and must be 1000 characters or fewer.", template, **context)
        consideration.authorized_attendance_units = None
        audit_action = "ATTENDANCE_CREDIT_REJECTED"
    consideration.status = action
    consideration.review_note = note
    consideration.reviewer_id = reviewer.id
    consideration.reviewer_role = reviewer.role
    db.session.add(VerificationAudit(actor_user_id=reviewer.id, actor_role=reviewer.role, target_type="AcademicActivity", target_id=activity.id, action=audit_action, note=note))
    db.session.commit()
    flash("Attendance credit granted." if action == "Authorized" else "Attendance consideration rejected.", "success")
    return redirect(request.path.rsplit("/attendance-consideration", 1)[0])


@app.route("/faculty/activities/<int:activity_id>/attendance-consideration/grant", methods=["POST"])
@role_required("faculty")
def faculty_grant_attendance_consideration(activity_id):
    activity = faculty_authorized_activity(activity_id)
    return decide_attendance_consideration(activity, db.session.get(User, session["user_id"]), "Authorized", "faculty_activity_detail.html", authorizations=AttendanceAuthorization.query.filter_by(activity_id=activity.id, faculty_id=session["user_id"]).all(), audits=VerificationAudit.query.filter_by(target_type="AcademicActivity", target_id=activity.id).all())


@app.route("/faculty/activities/<int:activity_id>/attendance-consideration/reject", methods=["POST"])
@role_required("faculty")
def faculty_reject_attendance_consideration(activity_id):
    activity = faculty_authorized_activity(activity_id)
    return decide_attendance_consideration(activity, db.session.get(User, session["user_id"]), "Rejected", "faculty_activity_detail.html", authorizations=AttendanceAuthorization.query.filter_by(activity_id=activity.id, faculty_id=session["user_id"]).all(), audits=VerificationAudit.query.filter_by(target_type="AcademicActivity", target_id=activity.id).all())


@app.route("/coordinator/activities/<int:activity_id>/attendance-consideration/grant", methods=["POST"])
@role_required("coordinator", "admin")
def coordinator_grant_attendance_consideration(activity_id):
    activity = db.get_or_404(AcademicActivity, activity_id)
    return decide_attendance_consideration(activity, db.session.get(User, session["user_id"]), "Authorized", "coordinator_activity_detail.html", audits=VerificationAudit.query.filter_by(target_type="AcademicActivity", target_id=activity.id).all())


@app.route("/coordinator/activities/<int:activity_id>/attendance-consideration/reject", methods=["POST"])
@role_required("coordinator", "admin")
def coordinator_reject_attendance_consideration(activity_id):
    activity = db.get_or_404(AcademicActivity, activity_id)
    return decide_attendance_consideration(activity, db.session.get(User, session["user_id"]), "Rejected", "coordinator_activity_detail.html", audits=VerificationAudit.query.filter_by(target_type="AcademicActivity", target_id=activity.id).all())
...
def approved_attendance_units(user_id, subject_id=None):
    query = AttendanceConsideration.query.filter_by(student_id=user_id, status="Authorized")
    if subject_id is not None:
        query = query.filter_by(subject_id=subject_id)
    return sum(item.authorized_attendance_units or 0 for item in query.all())


def attendance_summary(records, authorized_units=0):
    ...
    credited_units = min(max(int(authorized_units or 0), 0), missed)
    credited_attended = attended + credited_units
    percentage = round((credited_attended / total) * 100, 2) if total else None
    return {"total": total, "attended": attended, "missed": missed, "authorized_units": credited_units, "credited_attended": credited_attended, "percentage": percentage}
...
def render_attendance_page(selected_subject_id=None, form_error=None):
    ...
    records = []
    summary = {"total": 0, "attended": 0, "missed": 0, "authorized_units": 0, "credited_attended": 0, "percentage": None}
    if selected_subject is not None:
        records = Attendance.query.filter_by(
            user_id=user_id, subject_id=selected_subject.id
        ).order_by(Attendance.record_date.desc(), Attendance.id.desc()).all()
        summary = attendance_summary(records, approved_attendance_units(user_id, selected_subject.id))
    ...
    for subject in student_subjects:
        ...
        summaries.append({"subject": subject, "summary": attendance_summary(subject_records, approved_attendance_units(user_id, subject.id))})

    all_records = Attendance.query.filter_by(user_id=user_id).all()
    overall = attendance_summary(all_records, approved_attendance_units(user_id))
    return render_template(
        "attendance.html",
